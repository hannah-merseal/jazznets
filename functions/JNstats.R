...
JNstats <- function(){
  ...
  for(i in 1:61){
    ...
    loadFilePath <- paste("data/clean/JN_", i, "_clean.csv", sep = "")
    subject <- read.csv(loadFilePath)
    
    #pilot - calculate agreement on catch trials (*later: add agreement on reversed trials)
    ...
    info <- rbind(info, data.frame(participant, prolificID, NRemoved))
    saveFilePath <- paste("data/filtered/JN_", i, ".csv", sep = "")
